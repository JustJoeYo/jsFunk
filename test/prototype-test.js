...
const { findClubMembers } = require("../prototypes/problem-sets/clubs");

const { getStats } = require("../prototypes/problem-sets/mods");
...
describe("PROTOTYPES", () => {
  ...
  describe("Task Prompts", () => {
    it.skip("getAverageTime", () => {
      ...
      expect(completeAve).to.deep.equal(
        "The average time for all complete tasks is 87.5 minutes."
      );
      expect(inProgressAve).to.deep.equal(
  ...
  describe("Crafting Prompts", () => {
    it.skip("getSupplyList", () => {
      ...
      expect(crossStitchList).to.deep.equal([
        "fabric",
        "needle",
        "thread",
        "scissor",
